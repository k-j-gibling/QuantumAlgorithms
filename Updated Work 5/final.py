...
#TODO: Write out the form that the H_ijk_list can be.
def compute_effective_hamiltonian(H_ijk_list, stateVector):
    H_eff = 0

    """
        For instance H_ij may be of the form
            H_ij = X \otimes Z + Z \otimes X, which would yield the following H_ijk_list
            H_ijk_list = [['X','Z'],['Z','X']]
    """
    for element in H_ijk_list:

        current_matrix = pauli_dict[element[0]]
        expectation_product = 1

        for i in range(1, len(element)):
            currentPauli = pauli_dict[element[i]]
            currentExpectationValue = expectation_value(currentPauli,stateVector)
            expectation_product = expectation_product * currentExpectationValue


        H_eff = H_eff + (current_matrix * expectation_product)
...
def compute_effective_hamiltonian(H_list, stateVector):
    ...
    for element in H_list:
        ...
        for i in range(1, len(element)):
            currentPauli = pauli_dict[element[i]]
            currentExpectationValue = expectation_value(currentPauli,stateVector)
            currentProduct = currentProduct*currentExpectationValue

        H_eff = H_eff + pauli_dict[element[0]]*currentProduct
...
def partial_trace(rho, N, keep_qubits):
    ...
    rho = qt.Qobj(rho, dims=[dims_data,dims_data])
