...
class QuantumStateEvolver:
    ...
    def _compute_evolution_operator(self):
        ...
        # Use scipy's matrix exponential for better numerical stability
        U = expm(-1j * self.hamiltonian * self.evolution_time)
        
        # Ensure unitarity by orthogonalization if needed
        # U†U should be identity
        error = np.linalg.norm(U @ U.conj().T - np.eye(len(U)))
        
        if error > 1e-10:
            print(f"Warning: Evolution operator has unitarity error {error:.2e}")
            print("Performing polar decomposition to ensure unitarity...")
            # Polar decomposition: U = W @ sqrt(U† @ U)
            # We want just W (the unitary part)
    ...
    def _evolve_state(self):
        ...
        # Apply evolution using the pre-computed unitary
        try:
            qc.unitary(Operator(self.U_evolution), range(self.total_qubits), label='U(t)')
        except ValueError as e:
            print(f"Error creating unitary: {e}")
            print("Attempting alternative approach...")
            # Alternative: use append with explicit unitary gate
    ...
    def _measure_in_basis(self, basis_circuit):
        ...
        # Evolve using pre-computed unitary
        from qiskit.circuit.library import UnitaryGate
        gate = UnitaryGate(self.U_evolution, label='U(t)')
        qc.append(gate, range(self.total_qubits))
...
def example_simple_two_qubit():
    """
    Example: Very simple 2-qubit system for testing
    """
    print("\n" + "="*60)
    print("EXAMPLE: Simple 2-Qubit Evolution")
    print("="*60)
    ...
    # Simple Hamiltonian (Pauli X ⊗ Pauli X)
    X = np.array([[0, 1], [1, 0]])
...
if __name__ == "__main__":
    print("\nChoose example:")
    print("1. Bell state (4 qubits)")
    print("2. GHZ state (3 qubits)")
    print("3. Custom state (2 qubits)")
    print("4. Simple 2-qubit")
